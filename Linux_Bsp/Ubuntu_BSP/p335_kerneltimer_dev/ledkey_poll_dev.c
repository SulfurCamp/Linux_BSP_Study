...
static DECLARE_WAIT_QUEUE_HEAD(waitQueueRead); // read wait queue
...
static ssize_t ledkey_read(struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
	char kbuf=0;
	//kbuf=gpioKeyGet();
	//리턴할 값 없음 이벤트 발생이 없음 재우자!
	if(!(filp->f_flags & O_NONBLOCK)) // blocking 모드 open 여기서 재워야 해!
	{
		wait_event_interruptible(waitQueueRead, keyNum); // 잠들어서 이대로면 키번호가 안바뀔꺼야!
	// 잠들어서 이대로면 키번호가 안바뀔꺼야! + 조건이 참이 되거나 시간 초과가 발생할 때까지 sleep한다.
	//	wait_event_interruptible_timeout(waitQueueRead, keyNum, 100); 
	}
//	kbuf=(char)keyNum;
//	keyNum = 0;
	if(mutex_trylock(&keyMutex) != 0)
