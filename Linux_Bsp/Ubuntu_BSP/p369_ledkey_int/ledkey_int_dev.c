...
static irqreturn_t keyIsr(int irq, void * data) //data에는 NULL이 넘어오므로 0이 넘어온다.
{
	int i;
	for(i=0;i<GPIOCNT;i++)
	{
...
static int ledkey_init(void)
{
	...
	{
		result = request_irq(irqKey[i],keyIsr,IRQF_TRIGGER_RISING,irqName[i],NULL); //irqreturn_t keyIsr에 NULL이 data로 보내진다.
		if(result < 0 )
			return result;
	}
