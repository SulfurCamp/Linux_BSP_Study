...
static long ledkey_ioctl (struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch( cmd )
	{
		...
		case IOCTLTEST_LEDKEY:
    		if ((get_user(buff, (char *)arg)) != 0)
        		return -EFAULT;
    		gpioLedSet(buff);  // 받은 값으로 LED 출력
    		buff = gpioKeyGet();  // 현재 키 입력 읽기
    		if (copy_to_user((void *)arg, &buff, sizeof(buff)) != 0){
        		return -EFAULT;
			}
			break;
